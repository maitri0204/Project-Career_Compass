...
import { ArrowLeft, Brain, Calendar, GraduationCap, BookOpen, Briefcase, Download } from "lucide-react";
...
export default function AdminResultDetailPage() {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      const sObj = typeof result.student === "object" ? result.student : null;
      const sName = sObj
        ? [sObj.firstName, (sObj as any).middleName, sObj.lastName].filter(Boolean).join(" ")
        : "—";
      ...
      const infoFields = [
        ...
        { label: "Class / Grade", value: (sObj as any)?.classGrade || "—" },
        { label: "School Name", value: (sObj as any)?.schoolName || "—" },
      ...
      pdf.addPage(); await addImagePage("/3.png");
      pdf.addPage(); await addImagePage("/4.png");
      pdf.addPage(); await addImagePage("/5.png");
      ...
      pdf.text(pdf.splitTextToSize(subjStr, 58), 120, 109);
      ...
      y6 += 20;
      const colA = 20; const colB = 110;
      careers6.forEach((career, i) => {
        const xPos = i % 2 === 0 ? colA : colB;
        const yItem = y6 + Math.floor(i / 2) * 16;
        pdf.setFillColor(255, 255, 255);
      ...
      pdf.addPage(); await addImagePage("/7.png");
  ...
  const studentName = studentObj
    ? [studentObj.firstName, (studentObj as any).middleName, studentObj.lastName]
        .filter(Boolean)
        .join(" ")
    : "Student";
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
        ...
        Viewing result for: {studentName}
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-cyan-600 rounded-2xl p-8 mb-8 text-white">
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1">
            <p className="text-sm text-white/80 uppercase tracking-widest font-bold mb-1">Personality Type</p>
      ...
      <div className="rounded-2xl overflow-hidden shadow-sm border border-amber-100">
        ...
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-5 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-white">Career Pathway</h2>
